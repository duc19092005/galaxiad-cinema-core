...
- **Приём договора**: MovieManager загружает PDF, сканы и приложения партнёра; исходный файл неизменяем и сохраняется с hash и revision
- **OCR и анализ условий**: Python AI Service читает текстовый слой PDF или выполняет OCR на вьетнамском/английском через Tesseract, затем извлекает фильмы, сроки, область кинотеатров/форматов, классификацию и политику распределения выручки
