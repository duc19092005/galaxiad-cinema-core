...
*   **Xử lý Giữ Ghế Realtime (SSE + HTTP POST Seat Locking):** Khi khách hàng chọn ghế và tiến hành thanh toán, hệ thống sẽ **khóa ghế tạm thời trong 10 phút** thông qua SSE (Server-Sent Events) + HTTP POST. Ghế sẽ đổi trạng thái realtime trên màn hình của tất cả người dùng khác, loại bỏ hoàn toàn tình trạng **Double Booking** (trùng lặp ghế). Nếu quá thời gian 10 phút khách hàng không hoàn tất thanh toán, hệ thống tự động giải phóng ghế qua Background Job.
