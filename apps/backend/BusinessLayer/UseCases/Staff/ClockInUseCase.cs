...
public class ClockInUseCase
{
    ...
    public ClockInUseCase(IUnitOfWork unitOfWork, IConfiguration configuration)
    {
    ...
    public async Task<BaseResponse<ResClockInDto>> ExecuteAsync(ReqClockInDto dto)
    {
        ...
        var token = Jwt_helper.Encrypt(jwtKey, jwtIss, jwtAud, userEmail, username, dto.StaffId, userRoles, permissions);
