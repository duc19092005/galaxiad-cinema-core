...
public class SuspendContractUseCase
{
    ...
    public SuspendContractUseCase(IContractRepository repository, IUserContextService userContext)
    {
    ...
    public async Task<string> ExecuteAsync(Guid contractId, ReviewDecisionReqDto request, CancellationToken ct)
    {
        if (!_userContext.IsInRole("Admin"))
            throw new AppException("Chỉ Admin mới có quyền tạm dừng hợp đồng.", 403, "FORBIDDEN");
        ...
        return ContractStatus.Suspended.ToString();
    }
}
