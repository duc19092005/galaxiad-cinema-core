...
public class RejectMovieChangeRequestUseCase
{
    private readonly IContractRepository _repository;
    private readonly IUserContextService _userContext;

    public RejectMovieChangeRequestUseCase(IContractRepository repository, IUserContextService userContext)
    {
    ...
    public async Task<string> ExecuteAsync(Guid id, ReviewMovieChangeRequestDto request, CancellationToken ct)
    {
        if (!_userContext.IsInRole("Admin"))
            throw new AppException("Chỉ Admin mới có quyền từ chối yêu cầu thay đổi.", 403, "FORBIDDEN");
        ...
        if (item.Status != MovieChangeRequestStatus.PendingReview)
            throw new AppException("Chỉ yêu cầu PENDING_REVIEW mới được từ chối.", 409, "CONTRACT_STATE_CONFLICT");

        item.Status = MovieChangeRequestStatus.Rejected;
        item.ReviewNote = request.Reason;
        item.ReviewedByUserId = _userContext.GetUserId();
        item.UpdatedAt = DateTime.UtcNow;

        await _repository.SaveChangesAsync(ct);
        return MovieChangeRequestStatus.Rejected.ToString();
    }
}
