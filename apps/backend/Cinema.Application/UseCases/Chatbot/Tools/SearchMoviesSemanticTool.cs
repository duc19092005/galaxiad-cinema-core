...
public class SearchMoviesSemanticTool : IChatTool
{
    ...
    public async Task<string> ExecuteAsync(Dictionary<string, string> parameters)
    {
        ...
        {
            return JsonSerializer.Serialize(new { Message = "Không tìm thấy phim phù hợp với mô tả của bạn." });
        }
        ...
        if (movies.Count == 0)
        {
            return JsonSerializer.Serialize(new { Message = "Không có phim nào phù hợp với yêu cầu và trạng thái chiếu hiện tại." });
        }
