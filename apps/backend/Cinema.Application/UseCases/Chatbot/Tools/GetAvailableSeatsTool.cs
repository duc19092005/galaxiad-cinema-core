...
public class GetAvailableSeatsTool : IChatTool
{
    ...
    public async Task<string> ExecuteAsync(Dictionary<string, string> parameters)
    {
        ...
        if (string.IsNullOrWhiteSpace(movieName) && string.IsNullOrWhiteSpace(dateStr))
        {
            ...
            {
                Error = "Vui lòng cho biết tên phim hoặc ngày chiếu để tôi kiểm tra ghế trống."
            });
        }
        ...
        if (schedules.Count == 0)
        {
            return JsonSerializer.Serialize(new
            {
                Message = "Không tìm thấy suất chiếu phù hợp. Bạn có thể hỏi \"Phim ABC chiếu lúc mấy giờ?\" để xem lịch."
            });
        }
        ...
        {
            return JsonSerializer.Serialize(new { Message = "Không tìm thấy thông tin phòng chiếu." });
        }
